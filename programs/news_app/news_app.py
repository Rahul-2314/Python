...
# API key
api_key = "" #Use your own api key [newsapi.org]
...
popular_searches = ["All", "Cricket", "Movie", "Business", "Technology", "Health", "Sports", "Entertainment"]
query = st.selectbox("Choose a topic:", popular_searches)

# search query
search_query = query

# Date input (last two day unavailable)
today = datetime.now()
two_days_ago = today - timedelta(days=2)
twenty_days_ago = today - timedelta(days=20)
default_date = two_days_ago
selected_date = st.date_input("Choose a date", default_date, min_value=twenty_days_ago, max_value=two_days_ago)
            ...
            for idx, content in enumerate(contents, start=1):
                ...
                if title and content_text and "[removed]" not in title:
                    st.markdown(
                        ...
                        f"""
                        <div class="news-card">
                            <h2 class="news-title">Article {idx}: {title}</h2>
